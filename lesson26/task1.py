...
# нового контакта в качестве уникального идентификатора контакта. Когда мы добавляем новый контакт,
# то передаем такие аргументы в метод add_contacts: name, phone, email и favorite. Метод
# должен создать словарь с указанными ключами и значениями из параметров функции.
...
# Указанный словарь мы добавляем в список contacts. Не забываем увеличивать переменную current_id на
# единицу после каждого вызова метода add_contacts для сохранения уникальности ключа id для словаря.
#
